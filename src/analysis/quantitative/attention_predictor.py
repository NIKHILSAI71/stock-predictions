...
class AttentionPredictor:
    ...
    def prepare_features(self, stock_data: pd.DataFrame) -> np.ndarray:
        ...
        for period in [5, 10, 20]:
            ...
            features[f'ma_{period}_dist'] = ((close - ma) / (ma + 1e-10)).fillna(0)
        
        # RSI
        delta = close.diff()
        gain = delta.where(delta > 0, 0).rolling(14).mean()
        loss = (-delta).where(delta < 0, 0).rolling(14).mean()
        features['rsi'] = ((100 - (100 / (1 + gain / (loss + 1e-10)))) / 100).fillna(0.5)
        ...
        if 'Volume' in stock_data.columns:
            ...
            features['volume_ratio'] = (stock_data['Volume'] / (vol_ma + 1e-10)).clip(0, 3).fillna(1) / 3
        else:
    ...
    def predict(self, stock_data: pd.DataFrame) -> Dict[str, Any]:
        ...
        direction = "Bullish" if total_change > 0.3 else ("Bearish" if total_change < -0.3 else "Neutral")
        
        # Confidence based on attention concentration and trend clarity
        avg_attention = np.mean(all_attention_weights[-1], axis=(0, 1))
        attention_concentration = np.max(avg_attention) / (np.mean(avg_attention) + 1e-10)
        
        volatility = stock_data['Close'].pct_change().tail(20).std()
        confidence = max(35, min(80, 60 + attention_concentration * 5 - volatility * 300))
        
        # Get most attended time steps (interpretability)
        recent_attention = avg_attention[-20:]  # Last 20 days
        top_attention_days = np.argsort(recent_attention)[-3:][::-1]  # Top 3 most attended
...
def get_attention_prediction(
    ...
    predictor = AttentionPredictor(lookback=60, d_model=32, n_heads=4, n_layers=2)
