...
class LSTMPredictor:
    ...
    def _build_tf_model(self, input_shape: Tuple[int, int]) -> Any:
        ...
        model = Sequential([
            LSTM(self.hidden_size, return_sequences=True, input_shape=input_shape),
            Dropout(0.2),
    ...
    def predict(self, prices: np.ndarray) -> Dict[str, Any]:
        ...
        if self.use_tensorflow and self.model is not None:
            try:
                ...
                predictions = denormalize_data(pred_normalized, min_val, max_val)
            except Exception:
        ...
        return {
            ...
            "predicted_change_pct": float((predictions[4] - current_price) / current_price * 100),
            "direction": "Bullish" if predictions[4] > current_price else "Bearish",
...
def get_lstm_prediction(stock_data: pd.DataFrame, train_epochs: int = 10) -> Dict[str, Any]:
    """
    Main function to get LSTM prediction for a stock.
    ...
    train_result = predictor.train(prices[:-5], epochs=train_epochs, verbose=0)
