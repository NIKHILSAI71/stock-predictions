...
class SimpleGRUCell:
    ...
    def forward(self, x: np.ndarray, h_prev: np.ndarray) -> np.ndarray:
        ...
        h_candidate = self.tanh(self.Wh @ combined_reset + self.bh)
...
class GRUPredictor:
    ...
    def train(self, prices: np.ndarray, epochs: int = 10, verbose: int = 0) -> Dict[str, Any]:
        ...
        normalized = (prices - self.scaler_min) / (self.scaler_max - self.scaler_min + 1e-8)
        
        # Prepare sequences
        X, y = prepare_gru_sequences(normalized, self.lookback, 5)
    ...
    def predict(self, prices: np.ndarray) -> Dict[str, Any]:
        ...
        if self.use_tf and self.model is not None:
            try:
                ...
                pred_normalized = self.model.predict(seq_reshaped, verbose=0)[0]
                predictions = pred_normalized * (self.scaler_max - self.scaler_min) + self.scaler_min
            except Exception:
...
def get_gru_prediction(stock_data: pd.DataFrame, train_epochs: int = 10) -> Dict[str, Any]:
    """
    Main function to get GRU prediction for a stock.
...
def compare_lstm_gru(stock_data: pd.DataFrame) -> Dict[str, Any]:
    ...
    if gru_success and lstm_success:
        ...
        if gru_dir == lstm_dir:
            ...
            avg_confidence = (gru_result.get('confidence', 50) + lstm_result.get('confidence', 50)) / 2
            ...
            if gru_result.get('confidence', 50) > lstm_result.get('confidence', 50):
                combined_direction = gru_dir
                avg_confidence = gru_result.get('confidence', 50) * 0.8  # Reduce due to disagreement
            else:
