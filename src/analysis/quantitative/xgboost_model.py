...
def prepare_xgboost_features(
    ...
    low = stock_data['Low']
    volume = stock_data['Volume'] if 'Volume' in stock_data.columns else pd.Series(1, index=stock_data.index)
    ...
    if hasattr(stock_data.index, 'dayofweek'):
        features['day_sin'] = np.sin(2 * np.pi * stock_data.index.dayofweek / 5)
        features['day_cos'] = np.cos(2 * np.pi * stock_data.index.dayofweek / 5)
...
def get_xgboost_prediction(
    ...
    """
    Main function to get XGBoost prediction for a stock.
