...
async def get_fundamental_analysis(symbol: str):
    ...
    try:
        ...
        if info.get('operating_margins'):
            fundamentals['operating_margin'] = round(info['operating_margins'] * 100, 2)
        ...
        if info.get('dividend_yield'):
            fundamentals['dividend_yield'] = round(info['dividend_yield'] * 100, 2)
...
async def get_valuation_analysis(
    symbol: str,
    required_return: float = Query(0.10, description="Required rate of return (decimal)")
):
    ...
    try:
        ...
        # Current price
        current_price = info.get('current_price') or info.get('price_info', {}).get('current_price')
        ...
        if info.get('free_cash_flow') and info.get('beta'):
            try:
                ...
                if not growth_rate: growth_rate = 0.10
                ...
                terminal_value = (future_cash_flows[-1] * (1 + 0.03)) / (cost_of_equity - 0.03) # 3% Perp growth
                
                dcf_total = sum([fcf / ((1 + cost_of_equity) ** i) for i, fcf in enumerate(future_cash_flows, 1)])
        ...
        # Relative valuation
        def get_peg():
            if info.get('peg_ratio'): return info.get('peg_ratio')
...
@router.get("/qualitative/{symbol}")
async def get_qualitative_analysis(symbol: str):
    """Get qualitative fundamental analysis (ESG, Moat, Management)."""
    try:
