...
from src.analysis.technical import (
    ...
    find_support_resistance, pivot_points, fibonacci_retracement,
    volume_analysis
)
...
async def get_universal_signals(
    ...
    investment_horizon: str = Query("medium_term", enum=["short_term", "medium_term", "long_term"])
):
    """
    ...
    try:
        ...
        sr = find_support_resistance(stock_data)
        ...
        adaptive_weight = adaptive_system.get_sector_weight(sector)
        
        # Adjust confidence based on historical accuracy for this sector
...
async def get_trading_signals(
    ...
    try:
        ...
        # NEW: Get volume analysis for confirmation
        vol_analysis = volume_analysis(stock_data)
        volume_data = {
            "current_volume": vol_analysis.get('current_volume'),
            "avg_volume_20": vol_analysis.get('avg_volume_20'),
            "volume_ratio": vol_analysis.get('volume_ratio'),
            "volume_signal": vol_analysis.get('volume_signal'),
            "obv_trend": vol_analysis.get('obv_trend'),
            "mfi": vol_analysis.get('mfi'),
            "mfi_signal": vol_analysis.get('mfi_signal')
        }
        
        # NEW: Get pivot points for intermediate resistance/support levels
            ...
            try:
                ...
                if fcf > 0 and shares > 0:
                    ...
                    for year in range(1, 6):
                        stage1_value += (fcf_per_share * ((1 + long_term_growth) ** year)) / ((1 + discount_rate) ** year)
                    
                    terminal_fcf = fcf_per_share * ((1 + long_term_growth) ** 5) * (1 + 0.03)
        ...
        if dcf_intrinsic_value and dcf_intrinsic_value > 0:
            dcf_overvaluation_pct = round(((current_price - dcf_intrinsic_value) / dcf_intrinsic_value) * 100, 1)
        ...
        regime_data = regime_detection(returns, window=min(60, len(returns) - 1))
        ...
        if fair_value and fair_value > 0:
            overvaluation_pct = round((current_price - fair_value) / fair_value * 100, 1)
        ...
        macd_divergence = detect_macd_divergence(stock_data['Close'], macd_data['histogram'])
        ...
        pullback_target = signal_result.get('entry_recommendation', {}).get('pullback_target')
        pullback_pct = signal_result.get('entry_recommendation', {}).get('pullback_pct')
        pullback_status = "N/A"
        if pullback_target:
